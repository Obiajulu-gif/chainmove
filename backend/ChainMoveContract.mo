...
actor class ChainMoveContract() {
    ...
    public query func isRegisteredUser(caller : Principal) : async Bool {
    ...
    public query func getAllRegisteredUsers() : async [UserProfile] {
    ...
    // Check if the caller is a registered driver
    public query func isRegisteredDriver(caller : Principal) : async Bool {
    ...
    // Get user profile if the caller is a registered user
    public query func getUserProfile(caller : Principal) : async ?UserProfile {
        switch (isUser.get(caller)) {
            case (?true) userProfiles.get(caller);
            case (_) null;
        };
    };

    // Get driver profile if the caller is a registered driver
    public query func getDriverProfile(caller : Principal) : async ?DriverProfile {
        switch (isDriver.get(caller)) {
            case (?true) {
                switch (drivers.get(caller)) {
                    case (?driver) ?driver.profile; // Access profile from the Driver record
                    case null null;
                };
            };
            case (_) null;
