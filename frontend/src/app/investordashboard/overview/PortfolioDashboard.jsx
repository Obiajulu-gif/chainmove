...
const PortfolioDashboard = () => {
  const chartData = {
    labels: ["Jul 28", "Aug 1", "Aug 7", "Aug 14", "Aug 21", "Aug 28", "Sept 1"],
    datasets: [
      {
      ...
      <div className="flex-1 bg-gray-800 rounded-lg p-3">
        ...
        <div className="flex justify-center space-x-2 mt-3 text-gray-400">
          {["1D", "7D", "1M", "6M", "1Y"].map((period) => (
            <button key={period} className="hover:text-white focus:text-white text-sm">
              {period}
            </button>
          ))}
        </div>
      ...
      <div className="w-1/3 bg-gray-800 rounded-lg p-3 flex flex-col justify-between">
        ...
        <div className="mt-5">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-green-500 bg-green-900 px-2 py-0.5 rounded-full text-xs">↑ 15%</span>
          </div>
        </div>
      </div>
    </div>
